...
func NewSignalFxClient(opts metricswatcher.MetricsProviderOpts) (metricswatcher.MetricsProviderClient, error) {
	if opts.Name != metricswatcher.SignalFxClientName {
		return nil, fmt.Errorf("metric provider name should be %v, found %v", metricswatcher.SignalFxClientName, opts.Name)
	}
...
func (s signalFxClient) Name() string {
	return metricswatcher.SignalFxClientName
}

func (s signalFxClient) FetchHostMetrics(host string, window *metricswatcher.Window) ([]metricswatcher.Metric, error) {
	log.Debugf("fetching metrics for host %v", host)
	var metrics []metricswatcher.Metric
	...
	for _, metric := range []string{cpuUtilizationMetric, memoryUtilizationMetric} {
		...
		var fetchedMetric metricswatcher.Metric
...
func (s signalFxClient) FetchAllHostsMetrics(window *metricswatcher.Window) (map[string][]metricswatcher.Metric, error) {
	hostFilter := signalFxHostFilter + "*" + s.hostNameSuffix
	clusterFilter := signalFxClusterFilter + s.clusterName
	metrics := make(map[string][]metricswatcher.Metric)
	for _, metric := range []string{cpuUtilizationMetric, memoryUtilizationMetric} {
		uri, err := s.buildMetricURL(hostFilter, clusterFilter, metric, window)
		if err != nil {
			return metrics, err
...
func addMetadata(metric *metricswatcher.Metric, metricType string) {
	metric.Operator = metricswatcher.Average
	if metricType == cpuUtilizationMetric {
		metric.Name = cpuUtilizationMetric
		metric.Type = metricswatcher.CPU
	} else {
		metric.Name = memoryUtilizationMetric
		metric.Type = metricswatcher.Memory
	}
}

func (s signalFxClient) buildMetricURL(hostFilter string, clusterFilter string, metric string, window *metricswatcher.Window) (uri *url.URL, err error) {
...
*/
func getMetricsFromPayloads(metricData interface{}, metadata interface{}) (map[string]metricswatcher.Metric, error) {
	keyHostMap := make(map[string]string)
	hostMetricMap := make(map[string]metricswatcher.Metric)
	...
	for key, metric := range keyMetricMap {
		...
		fetchedMetric := metricswatcher.Metric{Value: sum / count}
