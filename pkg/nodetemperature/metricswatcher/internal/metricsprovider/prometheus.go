...
const (
	...
	promTempMetric     = "instance:node_temperature"
	allHosts           = "all"
...
func NewPromClient(opts metricswatcher.MetricsProviderOpts) (metricswatcher.MetricsProviderClient, error) {
	if opts.Name != metricswatcher.PromClientName {
		return nil, fmt.Errorf("metric provider name should be %v, found %v", metricswatcher.PromClientName, opts.Name)
	}
...
func (s promClient) Name() string {
	return metricswatcher.PromClientName
}

func (s promClient) FetchHostMetrics(host string, window *metricswatcher.Window) ([]metricswatcher.Metric, error) {
	var metricList []metricswatcher.Metric
	var anyerr error

	for _, method := range []string{promAvg, promStd} {
		for _, metric := range []string{promCpuMetric, promMemMetric, promTempMetric} {
...
func (s promClient) FetchAllHostsMetrics(window *metricswatcher.Window) (map[string][]metricswatcher.Metric, error) {
	hostMetrics := make(map[string][]metricswatcher.Metric)
	var anyerr error
...
func (s promClient) promResults2MetricMap(promresults model.Value, metric string, method string, rollup string) map[string][]metricswatcher.Metric {
	var metric_type string
	var operator string

	curMetrics := make(map[string][]metricswatcher.Metric)

	if metric == promCpuMetric {
		metric_type = metricswatcher.CPU
	} else if metric == promTempMetric {
		metric_type = metricswatcher.TEMPERATURE
	} else {
		metric_type = metricswatcher.Memory
	}

	if method == promAvg {
		operator = metricswatcher.Average
	} else if method == promStd {
		operator = metricswatcher.Std
	} else {
		operator = metricswatcher.UnknownOperator
	}

	switch promresults.(type) {
	case model.Vector:
		for _, result := range promresults.(model.Vector) {
			curMetric := metricswatcher.Metric{metric, metric_type, operator, rollup, float64(result.Value * 100)}
